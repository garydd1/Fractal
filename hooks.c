...
/*   Updated: 2023/04/16 17:58:17 by dgarizad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * @brief sets the position of each instance of each coin.
 * 
 * @param data 
 * @return int 
 */
int	ft_pick_coin(t_data *data)
{
	...
	while (i < data->map_h)
	{
		...
		while (j < data->map_w)
		{
			if (data->map_s[i][j] == 'C')
				coin_inst++;
			if(data->playerx == j && data->playery == i && data->img[C]->instances[coin_inst].z != -1)
			{
				data->coins--;
